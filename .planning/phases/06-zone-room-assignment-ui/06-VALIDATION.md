...
> Nyquist validation audit for Phase 6: zone-room-assignment-ui. Reconstructed
> from PLAN + SUMMARY artifacts (State B — no prior VALIDATION.md).
...
- **After every task commit:**
  `uv run pytest tests/test_websocket.py tests/test_phase06_acceptance.py -q`
...
Existing infrastructure covered all phase requirements — no Wave 0 installs
needed.

- `tests/test_websocket.py` — extended with 4 new zone WS tests (Phase 5) + 4
  new Phase 6 handler tests
- `tests/test_storage.py` — extended with
  `test_validate_zone_assignment_rejects_explicit_null`
- `tests/test_phase06_acceptance.py` — created by Nyquist audit (6 structural +
  1 smoke)

...
**Areas discussed:** Room card period badge — layout, style, and placement
...
**User's choice:** Same badge style as mode badge — consistent with existing mode badge pattern.
...
User raised: when global mode is off, room configuration is not applicable — should the period/off indicator come before the room mode badge?
...
**User's choice:** Period before mode badge — global state shown first, room config follows.

**Notes:** User requested ASCII previews of all states before deciding. Previews shown for: active time_program, custom room override, frost protection room mode, and global mode off.
...
**User's choice:** Gray badge showing "Off" — room mode badge still visible after it.

---

## Claude's Discretion

- Gray color for "Off" badge: use `--secondary-background-color` / `--secondary-text-color` (HA CSS variables per D-27).

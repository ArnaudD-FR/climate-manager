...
**Areas discussed:** Backend-computed status fields, Period badge colors & HA CSS, HA component policy for 2026.x

**Session intent:** Add architectural constraints — HA component usage policy and frontend/backend logic boundary — captured as D-23 through D-30 in CONTEXT.md.
...
**User's choice:** Custom hex values but defined once in `types.ts` (no inline hex literals)
...
**User's choice:** Specific replacement patterns at Claude's discretion per plan (but broken components documented in CONTEXT.md D-29)

---

## Claude's Discretion

- Specific native HTML replacements for broken HA components (within the policy of D-29)
- Whether `climate_entities` goes in `get_config` or a new `get_climate_entities` command

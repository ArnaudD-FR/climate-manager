...
**Date:** 2026-05-16 **Phase:** 1-Foundation **Areas discussed:** Project
scaffold, Config flow scope, Dev toolchain, Storage schema
...
| Build from scratch        | Write manifest.json, **init**.py, config_flow.py manually                              |          |

**User's choice:** Use integration_blueprint **Notes:** Strip CI actions; keep
file structure and pytest scaffold only.
...
**Room ID question:** User rejected UUID and slugified name options. Clarified
that HA has an Area Registry with stable `area_id` values — use those as room
IDs.

**Notes (freeform):**

- Auto-discovery model clarified during discussion: integration discovers all
  `climate.*` entities → groups by `area_id` → manages those areas
  automatically. Nothing manually configured.
- Sparse storage: only store values that differ from defaults. Default state =
  nothing in storage.
- Same pattern for persons: auto-discover all `person.*` entities, use
  `person.*` entity_id as person ID.
- UI ordering: rooms/persons with non-default config listed first in their
  respective tabs.

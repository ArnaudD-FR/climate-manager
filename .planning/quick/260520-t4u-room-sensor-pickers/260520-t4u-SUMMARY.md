...
Both fields save on blur via `ws.setRoomConfig` + `panel.reloadConfig()`. Empty
field sends `null` to the backend, which falls back to auto-detected sensors
(from `room_auto_sensors`) or the TRV `current_temperature` attribute. Updated

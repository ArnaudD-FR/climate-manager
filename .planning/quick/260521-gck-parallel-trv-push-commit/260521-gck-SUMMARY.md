...
Committed coordinator.py asyncio.gather refactor that replaces sequential TRV await loops, eliminating ~10s cumulative delay on mode changes with multiple rooms.

## What Was Done

Verified and committed the already-implemented parallel TRV push fix in `coordinator.py`. All five must_haves confirmed before committing:
...
Only `coordinator.py` was staged; all other modified/untracked files left untouched per constraints.
...
- [x] coordinator.py committed: 721786f confirmed via `git rev-parse --short HEAD`
- [x] Only coordinator.py staged (verified via `git status --short` before commit)

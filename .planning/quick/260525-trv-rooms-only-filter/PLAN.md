...
## Goal
Rooms that have only boiler/chaudière climate entities (no `current_temperature` attribute) must be excluded from:
1. The Rooms tab room list
2. The Persons tab room choices picker

## Detection strategy
A room is a "TRV room" if any of its climate entities has `current_temperature` in its state attributes.
...
- HA preserves last-known attributes on unavailable state, so offline TRVs stay tagged correctly
...
- Filter `statusRooms` to `.filter(r => r.has_trv !== false)` before building `allRoomIds`

# Requirements Archive: v1.3 Calendar Presence & Pre-heat
...
- [x] **CAL-01**: A person can be set to "Calendar" presence mode in the
...
- [x] **CAL-02**: The coordinator fetches `get_events` for each unique
...
- [x] **CAL-03**: In Scheduled mode, individual periods can have state
...
- [x] **CAL-04**: A per-person `wakeup_advance_minutes` value (default 60,
...
- [x] **PREHEAT-01**: User can enable or disable predictive pre-heat per room
  from the Rooms tab, with a configurable maximum lead time (default 120 min)
- [x] **PREHEAT-02**: When enabled, the coordinator starts heating the room
...
- [x] **PREHEAT-03**: The integration learns each room's thermal inertia from
...
- [x] **PREHEAT-04**: When a room is actively pre-heating, its status in the
...
- [x] **PREHEAT-05**: Pre-heat is compatible with even/odd week scheduling and
...
- [x] **MCALIB-01**: User can configure a room-level mapping from a Matter TRV
...
- [x] **MCALIB-02**: Matter state_changed listeners are registered and
...
- [x] **ARCH-02**: `room_mode` is removed entirely from coordinator, storage,
...
- [x] **OBS-01**: Structured log lines are emitted at INFO level on presence

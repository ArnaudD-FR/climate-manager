...
The `ha` label is opaque in the UI. Users don't know what "HA" means in this
context without reading documentation. It could mean "Home Assistant default",
"HA entity", or something else entirely.

## Solution

Rename the `ha` mode display label in the UI (the internal identifier can stay
`ha` if no config migration is needed). Candidate labels:
...
Recommended: **"HA person entity"** or **"Live detection (HA)"** — open for
decision before implementing.

This is a UI-only label change if the internal `mode: "ha"` string is kept as-is
in the config store. No migration needed in that case.

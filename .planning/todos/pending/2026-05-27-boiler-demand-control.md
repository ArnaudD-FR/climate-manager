...
Climate Manager should start the boiler when at least one room in a zone is
calling for heat, and stop it when all rooms are satisfied — avoiding
unnecessary boiler cycles and improving energy efficiency.
...
`hvac_action` is binary and only tells us _whether_ the TRV is calling for heat,
not _how much_. A room at 5% valve is treated the same as one at 100%.
...
- If `boiler_entity` is a `climate` entity → `climate.set_hvac_mode("heat")` /
  `("off")`

**Evaluation timing:**

- Demand is checked at each coordinator evaluation cycle
- Configurable debounce delay (default 3 min) prevents boiler flapping when
  rooms hover near their target temperature
...
- Compare `current_temperature` vs `target_temperature`: if
  `current < target - 0.5°C` → assume heating demand

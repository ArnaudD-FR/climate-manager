...
async function main() {
  ...
  async function expandFirstCard(componentTag) {
    await page.evaluate((tag) => {
      ...
      if (!tab) { console.warn('[screenshot] tab not found:', tag); return; }
      const isRooms   = tag.includes('rooms');
      const cardTag   = isRooms ? 'climate-manager-room-card' : 'climate-manager-person-card';
      const card = tab.shadowRoot?.querySelector(cardTag);
      if (!card) { console.warn('[screenshot] card not found:', cardTag); return; }
      const header = card.shadowRoot?.querySelector('.card-header-row');
      if (!header) { console.warn('[screenshot] .card-header-row not found in', cardTag); return; }
      header.click();
    }, componentTag);

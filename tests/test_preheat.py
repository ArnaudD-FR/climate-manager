...
async def test_preheat_reads_zone_enabled(hass):
    ...
    office_room = coord._rooms.get("office")
    if office_room is not None:
        office_room._frost_locked = False
        await office_room.compute_preheat(ctx)
    await hass.async_block_till_done()

    assert office_room is not None and office_room._preheat_active is True
    ...
    ctx2 = EvalContext(
        now=now,
        hass=hass,
        period_temperatures=config2["period_temperatures"],
    )
    office_room2 = coord2._rooms.get("office")
    if office_room2 is not None:
        office_room2._frost_locked = False
        await office_room2.compute_preheat(ctx2)
    await hass.async_block_till_done()

    assert office_room2 is None or office_room2._preheat_active is not True
...
async def test_preheat_default_zone_enabled(hass):
    ...
    hall_room = coord._rooms.get("hall")
    if hall_room is not None:
        hall_room._frost_locked = False
        await hall_room.compute_preheat(ctx)
    await hass.async_block_till_done()

    assert hall_room is not None and hall_room._preheat_active is True
    ...
    hall_room2 = coord2._rooms.get("hall")
    if hall_room2 is not None:
        hall_room2._frost_locked = False
        await hall_room2.compute_preheat(ctx2)
    await hass.async_block_till_done()

    assert hall_room2 is None or hall_room2._preheat_active is not True
